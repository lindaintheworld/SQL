...
ORDER BY SALARY DESC, 1 DESC; -- 첫번째 컬럼인 EMP_NAME이 다음 ORDER BY가 된다
...
-- GROUP BY 절*******************************
-- 같은 값들이 여러 개 기록된 커럼을 기준으로 같은 값들을 그룹 묶을 때 사용함
-- GROUP BY 컬럼명 | 계산식
-- 같은 항목 끼리 묶어서 계산할 때 사용함
-- 그룹 묶은 항목에 대한 계산은 SELECT 절에 그룹 함수로 명시됨
SELECT EMP_NAME, SALARY, DEPT_ID
FROM EMPLOYEE;
...
-- GROUP BY 절에 명시하지 않는 컬럼은 SELECT 절에 사용 못 한다!!!!!!!
-- SELECT 절에 그룹함수와 같이 사용할 컬럼은 반드시 GROUPD BY 절에 사용되어야 함
...
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '3', '남', '여') 성별,
            SUM(SALARY) 급여합계, ROUND(AVG(SALARY), -4), COUNT(*)
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '3', '남', '여');

-- ERROR : 출력행의 갯수가 달라서 에러남
...
FROM EMPLOYEE
GROUP BY DEPT_ID; --ERROR : ROW 갯수가 다름 DEPT_ ID는 7개인데 MAX 값은 값이 1개임

-- HAVING 절 ****************
-- GROUP BY 절 아래에 사용함
-- 반드시 GROUPD BY 절과 함께 사용해야 함
-- 그룹 묶어서 계산한 그룹함수 결과값들에 대한 조건 처리가 목적

-- HAVING 그룹함수(컬럼명) 비교연산자 비교값
-- SELECT 절은 HAVING 처리로 골라낸 결과만 출력 처리함

-- 부서별 급여합계 중 9백만원을 초과하는 부서와 급여합계 조회
SELECT DEPT_ID, SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_ID
...
SELECT EMP_NAME, SALARY,
            RANK() OVER (ORDER BY EMP_NAME DESC) 순위
FROM EMPLOYEE
ORDER BY 순위;
...
SELECT DEPT_ID, SUM(SALARY)
FROM EMPLOYEE
GROUP BY ROLLUP(DEPT_ID); -- 총합계를 더해서 맨 아래에 추가됨
...
-- 실습 : 부서코드와 직급코드를 함께 그룹으로 묶고 급여의 합계를 구함
-- ROLLUP 사용
